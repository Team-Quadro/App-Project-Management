<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    {{-- Menu Task Active khusus Admin --}}
                    @if(Auth::user()->isAdmin())
                    <x-nav-link :href="route('tasks.active')" :active="request()->routeIs('tasks.active')">
                        {{ __('Task Active') }}
                    </x-nav-link>
                    @endif
                </div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            {{-- Menu Task Active Mobile --}}
            @if(Auth::user()->isAdmin())
            <x-responsive-nav-link :href="route('tasks.active')" :active="request()->routeIs('tasks.active')">
                {{ __('Task Active') }}
            </x-responsive-nav-link>
            @endif
        </div>
