        ...
        <div class="flex h-full">
                ...
                <nav class="flex-1 overflow-y-auto sidebar-scroll px-2 py-2 space-y-0.5">
                    ...
                    @if (Auth::user()->isSuperAdmin())
                        <a href="{{ route('superadmin.dashboard') }}"
                           class="flex items-center gap-2.5 px-2.5 py-1.5 rounded-[4px] text-[13px] font-medium transition-colors duration-100 {{ request()->routeIs('superadmin.dashboard') ? 'bg-brand-50 text-brand-700' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-700' }}">
                            <svg class="w-4 h-4 {{ request()->routeIs('superadmin.dashboard') ? 'text-brand-500' : 'text-gray-400' }}" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M2.25 7.5h19.5m-19.5 9h19.5M6 3.75v16.5M18 3.75v16.5"/></svg>
                            Holding Dashboard
                        </a>
                        <a href="{{ route('superadmin.tenants.index') }}"
                           class="flex items-center gap-2.5 px-2.5 py-1.5 rounded-[4px] text-[13px] font-medium transition-colors duration-100 {{ request()->routeIs('superadmin.tenants.*') ? 'bg-brand-50 text-brand-700' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-700' }}">
                            <svg class="w-4 h-4 {{ request()->routeIs('superadmin.tenants.*') ? 'text-brand-500' : 'text-gray-400' }}" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M3.75 21h16.5M4.5 7.5h15M6 7.5V21m12-13.5V21M7.5 3h9l1.5 4.5H6L7.5 3z"/></svg>
                            Tenants
                        </a>
                    @endif
                    ...
                    <a href="{{ route('invitations.index') }}"
                       class="flex items-center gap-2.5 px-2.5 py-1.5 rounded-sm text-[13px] font-medium transition-colors duration-100 {{ request()->routeIs('invitations.*') ? 'bg-brand-50 text-brand-700' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-700' }}">
                        <svg class="w-4 h-4 {{ request()->routeIs('invitations.*') ? 'text-brand-500' : 'text-gray-400' }}" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6.75h16.5m-16.5 0v10.5a2.25 2.25 0 002.25 2.25h12a2.25 2.25 0 002.25-2.25V6.75m-16.5 0L12 13.5l8.25-6.75"/></svg>
                        Invitations
                    </a>
                </nav>
                ...
                <div class="border-t border-border p-2 shrink-0">
                    <div class="flex items-center gap-2 px-2 py-1">
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <p class="text-[11px] text-gray-400 truncate">{{ Auth::user()->role_label }}</p>
                        </div>
                    </div>
                    <div class="flex gap-1 mt-1.5">
                        <a href="{{ route('profile.edit') }}" class="flex-1 text-center text-[12px] font-medium text-gray-500 hover:text-brand-600 hover:bg-brand-50 rounded-sm py-1 transition-colors duration-100">Profile</a>
                        <form method="POST" action="{{ route('logout') }}" class="flex-1">
                            @csrf
                            <button type="submit" class="w-full text-[12px] font-medium text-gray-500 hover:text-red-600 hover:bg-red-50 rounded-sm py-1 transition-colors duration-100">Logout</button>
                        </form>
                    </div>
                </div>
            </aside>
            ...
            <div class="flex-1 flex flex-col min-w-0">
                <header class="h-12 bg-white border-b border-border flex items-center px-4 sm:px-6 shrink-0">
                    <button @click="sidebarOpen = !sidebarOpen" class="lg:hidden p-1 -ml-1 mr-3 rounded-sm text-gray-500 hover:bg-gray-100 hover:text-gray-900 transition-colors duration-100">
        ...
        <div x-data="toast()" x-init="init()" class="fixed top-3 right-3 z-100 flex flex-col gap-2 w-full max-w-xs pointer-events-none">
            <template x-for="(t, i) in toasts" :key="i">
                <div x-show="t.visible" class="pointer-events-auto toast-enter" :class="{ 'toast-leave': !t.visible }">
                    <div class="flex items-center gap-2 px-3 py-2 rounded-sm border bg-white shadow-sm text-[13px]"
