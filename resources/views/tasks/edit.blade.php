...
<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-1.5 text-[13px]">
            ...
            <span class="text-gray-300">/</span>
            <span class="font-medium text-gray-900">Edit Task</span>
        </div>
    </x-slot>
        ...
        <div class="v-card p-5">
            <form method="POST" action="{{ route('projects.tasks.update', [$project, $task]) }}">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="title" name="title" value="{{ old('title', $task->title) }}" required autofocus class="v-input" />
                    <x-input-error :messages="$errors->get('title')" class="mt-1" />
                </div>

                <div class="mb-4">
                    <label for="description" class="v-label">Description</label>
                    <textarea id="description" name="description" rows="3" class="v-input resize-none">{{ old('description', $task->description) }}</textarea>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
                    ...
                    <div>
                        ...
                        <input type="date" id="deadline" name="deadline" value="{{ old('deadline', $task->deadline?->format('Y-m-d')) }}" class="v-input" />
                        <x-input-error :messages="$errors->get('deadline')" class="mt-1" />
                    </div>
                </div>

                <div class="flex items-center gap-2 pt-4 border-t border-border">
                    <button type="submit" class="v-btn-primary text-[13px]">Update Task</button>
