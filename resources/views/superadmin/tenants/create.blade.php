...
<x-app-layout>
    <x-slot name="header">
        <h1 class="text-sm font-semibold text-ink">Create Subsidiary</h1>
    </x-slot>

    <div class="max-w-xl">
        <div class="v-card p-5">
            <form method="POST" action="{{ route('superadmin.tenants.store') }}" class="space-y-4">
                @csrf
                <div>
                    <label class="v-label" for="company_name">Subsidiary Name</label>
                    <input class="v-input" id="company_name" name="company_name" type="text" value="{{ old('company_name') }}" required />
                    <x-input-error :messages="$errors->get('company_name')" class="mt-1" />
                </div>
                <div>
                    <label class="v-label" for="industry">Industry</label>
                    <input class="v-input" id="industry" name="industry" type="text" value="{{ old('industry') }}" />
                ...
                <div class="flex items-center gap-2 pt-4 border-t border-hairline">
                    <button type="submit" class="v-btn-primary text-[13px]">Create Subsidiary</button>
                    <a href="{{ route('superadmin.tenants.index') }}" class="v-btn-secondary text-[13px]">Cancel</a>
                </div>
            </form>
        </div>
    </div>
